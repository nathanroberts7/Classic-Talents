...
public class RHSideButtons {
    ...
    fileprivate let buttonSize = CGSize(width: UIScreen.main.bounds.height/16,
                                        height: UIScreen.main.bounds.height/16)
    fileprivate let verticalSpacing = CGFloat(10)
...
extension RHSideButtons: RHButtonViewDelegate {
    ...
    public func didSelectButton(_ buttonView: RHButtonView) {
        if let indexOfButton = buttonsArr.firstIndex(of: buttonView) {
            delegate?.sideButtons(self, didSelectButtonAtIndex: indexOfButton, menuType: menuType)
